        ...
        public async Task<ClearanceFormDto> Handle(GetClearanceFormByIdQuery request, CancellationToken cancellationToken)
        {
           var entity = await _repository.GetByIdAsync(request.Id);
            ...
            var clearanceFormDto = new ClearanceFormDto
            {
                ...
                Signatures = entity.ClearanceSignatures.Select(s => new ClearanceSignatureDto
                {
                    Id = s.Id,
                    ...
                    SignedByFacultyName = s.SignedByFaculty?.EmployeeId ??
                                          ( $"{s.SignedByFaculty?.FirstName} " +
                                            $"{s.SignedByFaculty?.LastName}").Trim() ??"N/A",
